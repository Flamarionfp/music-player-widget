...
export const TrackInfo = (props: TrackInfoProps) => {
  ...
  return (
    <div className={`flex gap-7 ${containerStyle}`}>
      <img className={getImageStyle()} alt={trackImageAlt} src={trackImagePath} />
      <header className="w-full flex flex-col justify-center">
        <h1 className="font-bold text-2xl text-[#E1E1E6]">{truncate(trackName, 20)}</h1>
        <h2 className="font-extralight text-lg text-[#E1E1E6]">{truncate(artistName, 30)}</h2>
      </header>
    </div>
  );
};
